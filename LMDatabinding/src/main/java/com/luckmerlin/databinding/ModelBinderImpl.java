 ...
 class ModelBinderImpl {
    ...
    public MatchBinding bindModelForMatchBinding(Context context,MatchBinding matchBinding,String debug){
        if (null!=matchBinding){
            if (null==matchBinding.getCurrent()){//If model not create
                ...
                LModel model=null!=createdModel?createdModel.mModel:null;
                if (null==model){
                    return null;
                }
    ...
    public MatchBinding bindModelForObject(Context context, Object object, String debug){
        if (null==object||!DataBindingUtil.checkDataBindingEnable(false)){
            ...
            if (null==matchBinding&&classFinder.isExistClass((Class)object,LModel.class.getName())){//If need create by model class
        ...
        }else if (object instanceof LModel){
            LModel lModel=(LModel)object;
            if (lModel instanceof OnModelResolve&&!lModel.isRootAttached()){//If not attached
                ...
                return null!=modelObj?bindModelForObject(context,modelObj,debug):null;
            }
            return null;
            ...
            if (null!=modelObj){
                ...
                if (object instanceof Activity){
                    ...
                    LModel model=null!=currentModel&&currentModel instanceof LModel?(LModel)currentModel:null;
