...
public final class ILocalFile extends Document {
    private transient Reply<IPath> sync;

    public ILocalFile(String parent, String name, String extension, String title, String imageUrl, int childCount,
    ...
    public static ILocalFile build(File file, String imageUrl, Md5Reader reader){
        ...
        if (null!=path){
            ...
            return new ILocalFile(path.getParent(),path.getName(false),path.getExtension(),
    ...
    public boolean applySync(Reply<IPath> sync){
        this.sync=sync;
        return true;
    }
    ...
    private ILocalFile(Parcel dest){
        if (null!=dest){
    ...
    public static final Creator<ILocalFile> CREATOR = new Creator<ILocalFile>() {
        public ILocalFile createFromParcel(Parcel in) {
            return new ILocalFile(in);
        }

        @Override
        public ILocalFile[] newArray(int size) {
            return new ILocalFile[size];
        }
    };
